...
import { eceResearchPapers } from '@/Json/ConferencePapers';

const ECEConferencePapers = () => {
  ...
  const currentEntries = eceResearchPapers.slice(indexOfFirstEntry, indexOfLastEntry);
  const totalPages = Math.ceil(eceResearchPapers.length / entries);
  ...
  return (
    <div className="py-10 rounded-lg">
      <h3 className='text-4xl font-novaBold text-center mb-5'>ECE Conference Papers</h3>
        ...
        <tbody className="border-t-2 rounded-lg text-xs lg:text-sm font-novaSemi">
          {currentEntries.map((entry, index) => (
            ...
            >
              ...
              <td className="p-4 border-b border-l border-white border-opacity-20">{entry.authorName}</td>
      ...
      <div className="flex justify-between max-sm:flex-col mt-5">
        <div className="text-sm mb-2.5 mr-5 pt-3 text-gray-700">
          Showing {indexOfFirstEntry + 1} to {Math.min(indexOfLastEntry, eceResearchPapers.length)} of {eceResearchPapers.length} entries
        </div>
...
export default ECEConferencePapers;
