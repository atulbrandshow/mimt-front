...
import { ceResearchPapers } from '@/Json/ConferencePapers';

const CEConferencePapers = () => {
  ...
  const currentEntries = ceResearchPapers.slice(indexOfFirstEntry, indexOfLastEntry);
  const totalPages = Math.ceil(ceResearchPapers.length / entries);
  ...
  return (
    <div className="py-10 rounded-lg">
      <h3 className='text-4xl font-novaBold text-center mb-5'>CE Conference Papers</h3>
      ...
      <div className="flex justify-between max-sm:flex-col mt-5">
        <div className="text-sm mb-2.5 mr-5 pt-3 text-gray-700">
          Showing {indexOfFirstEntry + 1} to {Math.min(indexOfLastEntry, ceResearchPapers.length)} of {ceResearchPapers.length} entries
        </div>
...
export default CEConferencePapers;
