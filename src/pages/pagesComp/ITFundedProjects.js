...
import { itProject } from '@/Json/FundedProjects';

const ITFundedProjects = () => {
  return (
    <div className="py-10 rounded-lg">
      <h3 className='text-4xl font-novaBold text-center mb-5'>IT Funded Projects</h3>
      <table className="min-w-full bg-white rounded-lg">
        ...
        <tbody className="border-t-2 rounded-lg text-xs lg:text-sm font-novaSemi">
          {itProject.map((entry, index) => (
            <tr
              key={index}
              className={`bg-indigo-950 text-white ${index === itProject.length - 1 ? 'rounded-bl-lg rounded-br-lg' : ''}`}
            >
              <td className={`p-4 border-b border-white border-opacity-20 ${index === itProject.length - 1 ? 'rounded-bl-lg' : ''}`}>
                {index + 1}
              </td>
...
export default ITFundedProjects;
