...
import { eceJournalConnect } from '@/Json/Journals';
import Link from 'next/link';

const ECEDepartmentJournals = () => {
  const [entries, setEntries] = useState(10);
  const [currentPage, setCurrentPage] = useState(1);

  const indexOfLastEntry = currentPage * entries;
  const indexOfFirstEntry = indexOfLastEntry - entries;
  const currentEntries = eceJournalConnect.slice(indexOfFirstEntry, indexOfLastEntry);
  const totalPages = Math.ceil(eceJournalConnect.length / entries);
  ...
  return (
    <div className="py-10 rounded-lg">
      <h3 className='text-4xl font-novaBold text-center mb-5'>ECE Department Journals</h3>
      ...
      <div className="flex justify-between max-sm:flex-col mt-5">
        <div className="text-sm mb-2.5 mr-5 pt-3 text-gray-700">
          Showing {indexOfFirstEntry + 1} to {Math.min(indexOfLastEntry, eceJournalConnect.length)} of {eceJournalConnect.length} entries
        </div>
...
export default ECEDepartmentJournals;
