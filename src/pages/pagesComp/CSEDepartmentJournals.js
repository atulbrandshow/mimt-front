...
import { cseJournalHub } from '@/Json/Journals';
import Link from 'next/link';

const CSEDepartmentJournals = () => {
  const [entries, setEntries] = useState(10);
  const [currentPage, setCurrentPage] = useState(1);

  const indexOfLastEntry = currentPage * entries;
  const indexOfFirstEntry = indexOfLastEntry - entries;
  const currentEntries = cseJournalHub.slice(indexOfFirstEntry, indexOfLastEntry);
  const totalPages = Math.ceil(cseJournalHub.length / entries);
  ...
  return (
    <div className="py-10 rounded-lg">
      <h3 className='text-4xl font-novaBold text-center mb-5'>CSE Department Journals</h3>
      ...
      <div className="flex justify-between max-sm:flex-col mt-5">
        <div className="text-sm mb-2.5 mr-5 pt-3 text-gray-700">
          Showing {indexOfFirstEntry + 1} to {Math.min(indexOfLastEntry, cseJournalHub.length)} of {cseJournalHub.length} entries
        </div>
...
export default CSEDepartmentJournals;
