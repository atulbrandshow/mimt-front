...
import { eceProject } from '@/Json/FundedProjects';

const ECEFundedProjects = () => {
  return (
    <div className="py-10 rounded-lg">
      <h3 className='text-4xl font-novaBold text-center mb-5'>ECE Funded Projects</h3>
      <table className="min-w-full bg-white rounded-lg">
        ...
        <tbody className="border-t-2 rounded-lg text-xs lg:text-sm font-novaSemi">
          {eceProject.map((entry, index) => (
            <tr
              key={index}
              className={`bg-indigo-950 text-white ${index === eceProject.length - 1 ? 'rounded-bl-lg rounded-br-lg' : ''}`}
            >
              <td className={`p-4 border-b border-white border-opacity-20 ${index === eceProject.length - 1 ? 'rounded-bl-lg' : ''}`}>
                {index + 1}
              </td>
...
export default ECEFundedProjects;
