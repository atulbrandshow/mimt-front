...
export default function Page({ data }) {
  const p = data?.pageData || {}; // ✅ Fallback empty object

  // ✅ Safe recruiter list – no crash even if p = undefined
  const recruiters = Array.from({ length: 15 }).map((_, i) => ({
    img: p[`SliderImage_${i + 1}`] || null,
  ...
  const total = activeRecruiters.length;
  ...
  useEffect(() => {
    if (total === 0) return; // ✅ Prevent interval error
