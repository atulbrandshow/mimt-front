    ...
    <div className="bg-white">
      ...
      <section className="w-[88vw]">
        <div className="bg-primary rounded-r-[50px]">
          <div className="max-w-[1500px] ml-auto py-16 px-6 md:px-16">
      ...
      {cardsArray.map((card, idx) => (
        ...
        >
          ...
          <div className="relative z-10 mt-6 space-y-2 group-hover:text-white transition-colors">
            <h3 className="text-lg font-novaSemi">{card.Card_Name}</h3>
            <p className="text-sm opacity-80 font-novaReg">{card.Card_Course}</p>
            {card.Card_Company && (
              <p className="text-sm font-novaReg text-[#4ea8ff] group-hover:text-[#b7d7ff]">
                {card.Card_Company.replace("Company :", "").trim()}
              </p>
            )}
            <p className="text-sm font-novaBold text-green-600 group-hover:text-green-300">
              {card.Card_CTC}
            </p>
          </div>
          <p className="relative z-10 mt-4 text-sm font-novaReg text-gray-500 px-3 group-hover:text-gray-200 transition-colors">
