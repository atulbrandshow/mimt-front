    ...
    <div className="bg-white">
      ...
      <section className="w-full max-w-[1600px] mx-auto grid grid-cols-12 py-20 max-sm:py-2 gap-10 px-3 sm:px-6 max-sm:gap-0">
        <div className="col-span-9 max-xl:col-span-8 max-lg:col-span-12">
          <div dangerouslySetInnerHTML={{ __html: data?.pageData?.StudentCertificateDesc }} className={descriptionCss} />
          <section className="py-12 bg-white">
            <div className="max-w-7xl mx-auto ">
              {/* Gallery Grid */}
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <MIMTGallery data={data?.pageData?.StudentCertificateGallery} />
              </div>
            </div>
          </section>
        </div>
