...
const MediumHighlightBannerSlider = ({
    banners = [],
}) => {
            ...
            >
                {banners.map((banner) => (
                    <SwiperSlide key={banner._id}>
                        <div
                            className="relative overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 cursor-pointer group"
                        ...
                        >
                            ...
                            {(banner.title || banner.description) && (
                                <div className="absolute bottom-6 left-0 right-0 p-4 text-white transform translate-y-0 transition-transform duration-300">
                                    {banner.title && <h3 className="ml-0 md:ml-10 lg:ml-20 font-novaBold text-xl md:text-2xl py-0.5 md:py-3 line-clamp-1">{banner.title}</h3>}
...
export default MediumHighlightBannerSlider
